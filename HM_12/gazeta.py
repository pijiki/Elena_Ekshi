...
for name_cat in list_category:
    num_cat = list_category.index(name_cat)+1
    if num_cat == num_category:
        CATEGORY = str(name_cat).split('/')[2]
...
for article in articles:
    ...
    publication = re.match(r'(\d+ \w+ \d+,|\w+,|\d+ \w+) \d\d:\d\d',
                           publication).group()
